    ...
    def update_edge_vectors(self):
        ...
        for curr_idx in range(n):
            ...
            for curr_ng_edge_label in curr_neighbor_edge_labels:
                curr_tuple = tuple([curr_edge_label, curr_ng_edge_label])
                curr_tuple_swap = tuple([curr_ng_edge_label, curr_edge_label])
                #
                curr_edge_label_pair_consistency = None
                if curr_tuple in self.edge_label_consistency_matrices:
                    curr_edge_label_pair_consistency = self.edge_label_consistency_matrices[curr_tuple]
                elif curr_tuple_swap in self.edge_label_consistency_matrices:
                    curr_edge_label_pair_consistency = self.edge_label_consistency_matrices[curr_tuple_swap]
                else:
                    raise AssertionError
                #
                curr_ng_edge_label_matrix = self.edge_label_matrices[curr_ng_edge_label]
                #
                # order is important
                curr_edge_label_pair_consistency = np.dot(curr_edge_label_pair_consistency, curr_ng_edge_label_matrix)
                #
                if curr_edge_label_pair_consistencies is None:
                    curr_edge_label_pair_consistencies = np.copy(curr_edge_label_pair_consistency)
                else:
                    curr_edge_label_pair_consistencies += curr_edge_label_pair_consistency
            #
            if self.is_curr_edge_label_matrix_in_update:
                curr_edge_label_pair_consistencies += self.edge_label_matrices[curr_edge_label]
            #
            curr_edge_label_matrix_new = curr_edge_label_pair_consistencies - curr_edge_label_local_consistency
            if debug:
                print 'curr_edge_label_matrix_new', curr_edge_label_matrix_new
            #
...
if __name__ == '__main__':
    ...
    if is_load:
        ...
        # normalization kind of doesn't seem to make sense
